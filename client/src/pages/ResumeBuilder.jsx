...
import { ArrowLeftIcon, User, Sparkles, FolderIcon, GraduationCap, Briefcase, FileText, ChevronLeft, ChevronRight, Share2Icon, EyeIcon, EyeOffIcon, DownloadIcon } from "lucide-react";
...
export default function ResumeBuilder() {
  ...
  const loadExistingResume = async () => {
    ...
    if(data.resume){
      ...
      document.title=data.resume.titile;
    }
  ...
  const handleShare=()=>{
    const frontenUrl=window.location.href.split('/app/')[0];
    const resumeUrl=frontenUrl+'/view/'+resumeId;
...
const {data} = await api.put('/api/resumes/update',formData,{headers:{
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1">
              ...
              <div className="space-y-6">
                ...
                {activeSection.id === "summary" && (
                  <ProfessionalSumaaryForm
                    data={resumeData.professional_summary}
                    onChange={(professional_summary) => setResumeData((prev) => ({ ...prev, professional_summary }))}
                ...
                {activeSection.id === "experience" && (
                  <ExperienceForm
                    ...
                    onChange={(experience) => setResumeData((prev) => ({ ...prev, experience: experience }))}
                  />
                )}
                {activeSection.id === "education" && (
                  <EducationForm data={resumeData.education} onChange={(education) => setResumeData((prev) => ({ ...prev, education: education }))} />
                )}
                {activeSection.id === "projects" && (
                  <ProjectForm data={resumeData.project} onChange={(projects) => setResumeData((prev) => ({ ...prev, project: projects }))} />
                )}
                {activeSection.id === "skills" && (
                  <SkillsForm data={resumeData.skills} onChange={(skills) => setResumeData((prev) => ({ ...prev, skills: skills }))} />
                )}
              </div>
  ...
  justify-end gap-2"
      ...
      rounded-lg ring-blue-300 hover:ring transition-colors"
                  ...
                  <button onClick={chnageResumeVisibility} className='flex items-center p-2 px-4 gap-2 text-xs
